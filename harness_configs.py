"""Read-only surfacing of native harness model configs (M-UX.2, C1/C2).

The release gate pilot found that grok and opencode each decide which model
...
and the per-harness "Default Model" label to display with source-path attribution.
It NEVER writes a harness-owned file (editing is P4, the spec's read-first
...
@dataclass
class ModelEntry:
    """One model a harness can run, as named in its OWN config.

    ``key`` is the identifier the harness uses (a grok ``[model.<key>]`` name, an
...
class HarnessModelConfig:
    """A parsed read-only view of one harness's native model config.

    ``source_path`` is the absolute file the data came from (for the UI's
    "edited in the harness's own config" attribution). ``exists`` is whether that
...
def parse_grok_config(text, *, source_path=''):
    """Parse grok's ``config.toml`` text into an ``HarnessModelConfig``.
    ...
    """
    cfg = HarnessModelConfig(harness_id='grok', source_path=source_path)
...
def load_grok_config(*, home=None, path=None):
    """Load + parse grok's config from disk (defensive). Returns HarnessModelConfig.
    ...
    try:
        ...
        return HarnessModelConfig(harness_id='grok', source_path=src, exists=False)
...
# surfaces it on the Harnesses page. Three states → three strings.
...
def parse_opencode_config(text, *, source_path=''):
    """Parse opencode's JSON config into an ``HarnessModelConfig``.
    ...
    raises.
    """
    cfg = HarnessModelConfig(harness_id='opencode', source_path=source_path)
    if not text or not text.strip():
        return cfg
...
def load_opencode_config(*, home=None, path=None):
    """Load + parse opencode's config from disk (defensive). Returns HarnessModelConfig.
...
# ── M-UX.1: the effective default harness's "model story" label ─────────────────

def default_model_label(settings, *, home=None, native_label=None):
    """The truthful "Default Model" label for the EFFECTIVE default harness (C2).
    ...
    agent that actually decides the model for new sessions:
      ...
      * claude (or any harness with no native model config) → ``native_label``
        (Anthropic/provider text — claude's model story is the providers
        surface, unchanged);
      * grok / opencode → ``"Managed by <Display> (<config path>)"`` plus the
        ...
        the harness + path attribution still hold, so the row never lies about
    ...
    """
    import harnesses as _agents
    harness_id = getattr(settings, 'harness_default', '') or _agents.DEFAULT_HARNESS
    return _label_for_agent(
        harness_id, settings=settings, project_path='',
        home=home, native_label=native_label)
...
def default_model_label_for(settings, project_path, *, home=None,
    ...
    the maintainer's second reveal: a claude-OVERRIDE project's Provider submenu wore the
    GLOBAL default harness's label — "Default (Managed by Grok Build…)" on a row
    that actually runs claude — because the window computed ONE label from
    ``harness_default`` and pushed it to every row. This resolves the project's
    EFFECTIVE agent (``settings.effective_harness(project_path)`` — per-project
    override beats the global default) and labels THAT harness's model story, so a
    claude-override row reads claude's native/provider label exactly as it would when
    claude IS the global default, and a follow-default row matches
    ``default_model_label`` (the global default harness). Pure + defensive.
    """
    harness_id = settings.effective_harness(project_path)
    return _label_for_agent(
        harness_id, settings=settings, project_path=project_path or '',
        home=home, native_label=native_label)


def _label_for_agent(harness_id, *, settings=None, project_path='',
                     home=None, native_label=None):
    """Shared label body for ``default_model_label`` (global) and
    ``default_model_label_for`` (per-row). Given a resolved ``harness_id``,
    ...
    import harnesses as _agents
    adapter = _agents.ADAPTERS.get(harness_id)
    display = adapter.display_name if adapter is not None else harness_id

    cfg = load_harness_config(harness_id, home=home)
    if cfg is None:
        # Claude: when Settings names a non-native default (or a pin), label
        # THAT provider. Empty → Anthropic native (honors ``native_label`` so
        # tests can inject).
    ...
    # FB-1b: NO declared default key. The truth is the harness's BUILT-IN default
...
# claude provider models — the no-subscription grok flow dead-ended. This surfaces the
# EFFECTIVE harness's NATIVE models for the per-project Provider submenu:
...
# harness's built-in, invisible to the config, so we never fake an active marker).

# Suffix appended to the model whose key matches the config's declared default.
DEFAULT_MARKER = ' • default'   # " • default"


def native_model_options(harness_id, *, home=None, cfg=None):
    """``(ids, labels)`` parallel lists of ``harness_id``'s NATIVE models, or
    ``None`` for a harness with no native model-config surface (claude/unknown —
    that picker stays the Claude provider list, byte-identical to today).

    ``ids`` are the values written into ``model_pins[path]`` and passed to
    ...
    ``([], [])`` — the harness IS native but has no parseable models, so the
...
# ── B2 / M-UX.13: per-harness account / auth status (PRESENCE-BASED) ────────────
#
# Constitution C1/C2 + triage-2 S2/S3/S9: the Harnesses page showed grok/opencode
...
# status line per harness on the Harnesses page, driven by PRESENCE of the harness's
