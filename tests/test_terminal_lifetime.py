...
test_lifecycle.py / test_session_harnesses.py). The full restore→override→spawn
...
def _fake_tv(*, child_pid=4242, explicit_agent='grok', is_zellij=False,
    ...
    t = types.SimpleNamespace(
        ...
        _explicit_harness=explicit_agent,
...
def test_natural_exit_clears_explicit_harness():
    """T-d/T-a: a non-zellij child exit (natural quit OR deactivate-via-SIGTERM)
    clears _explicit_harness — so the NEXT spawn follows the pending override, not
    ...
    assert t._explicit_harness is None
...
def test_spawn_failure_end_clears_explicit_harness():
...
def test_zellij_detach_preserves_explicit_harness(monkeypatch):
    ...
    t = _fake_tv(explicit_agent='grok', is_zellij=True, zellij_session='pm-x')
    TerminalView._fire_exit_if_current(t, 4242, 0)
    assert t._explicit_harness == 'grok'      # preserved through detach
...
def test_stale_pid_does_not_clear():
    ...
    assert rv is False
    assert t._explicit_harness == 'grok'      # untouched
