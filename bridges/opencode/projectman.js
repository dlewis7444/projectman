...
// versions (e.g. permission.asked vs permission.updated; the laptop dev build
// and the VM gate build differ). The maps below react to every spelling we've
// seen; an unrecognised event is simply ignored. The plugin instance is
// per-directory — `input.directory`/`worktree` is the cwd ProjectMan spawned
// opencode in — so the cwd is taken from the closure, not parsed from events.
